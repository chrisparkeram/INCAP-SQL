...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-05-2024 a las 16:54:30
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `actualizar_credito` ()   BEGIN
UPDATE cartera SET pagada='s'
WHERE saldo<=0;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `actualizar_netocartera` ()   BEGIN
DECLARE var_codfac int;
DECLARE var_neto decimal(10,0);
DECLARE var_final int DEFAULT 0;
DECLARE c1 CURSOR FOR SELECT factura_cod,SUM(neto) FROM factura_detalle GROUP BY factura_cod;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `actualiza_entrada_detalle` ()   BEGIN
...
-- busca factura en cartera
	 UPDATE cartera SET total_neto=var_neto
	 WHERE cod_factura=var_codfac;
 	END LOOP bucle;
	 SELECT cod_factura, neto FROM cartera;
 	CLOSE c1;
...
OPEN c1;
bucle:LOOP
    FETCH c1 INTO var_producod, var_cantidad;-- empieza desde el registro de consulta factura_detalle
    IF var_final=1 THEN
    LEAVE bucle;
    END IF;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `contar_producto` (IN `dato` VARCHAR(50), OUT `total` INT UNSIGNED)   BEGIN
SET total=(
    SELECT COUNT(*)
    FROM productos 
...
bucle:LOOP
   ...
   LEAVE bucle;
   END IF;
   -- instrucciones a realizar en procedure con cursor
   IF var_dias>0 AND var_saldo>0 THEN
   UPDATE cliente SET activo="I"
   WHERE cod_cliente=var_codcli;
   END IF;
END LOOP bucle;
-- consultar tabla clienepara verificar incactivacion
...
DECLARE edadp int;
DECLARE i int;
-- seccion de creacion de nombre de cursor
DECLARE c1 CURSOR FOR SELECT edad FROM cliente;
-- seccion de asignacion de valores a variable 
SET suma=0;-- acomular o totalizador de edades
SET i=1;-- contador ciclo while
SET total=0;-- contador de cantidad de clientes en tabla
SELECT COUNT(*) INTO total FROM cliente;-- para hallar la cantidad de registros clienes en tabla
-- apertura cursor
OPEN c1;
-- seccion de ciclo while 
WHILE i<=total DO 
FETCH c1 INTO edadp;-- revisar cada fila de la consulta de conteo de registro
SET suma= suma+edadp;-- totalizador de la edades 
...
-- seccion de creacion de curso para consulta select
DECLARE c1 CURSOR FOR SELECT t1.empleado_cod, t2.nombre1, t2.apellido1, t1.neto_pagar
FROM nomina t1 INNER JOIN empleado t2 ON 
t1.empleado_cod, t2.cod_empleado;
SET mayor=0;
SET i=1;
SET total=0;
-- conteo de registros en tabla de nomina
SELECT COUNT(*) INTO total FROM nomina;
-- apertura cursor
OPEN c1;
WHILE 1<=total DO
-- variables deben estar en el mismo orden de la consulta select en declare cuursor
...
SET mayor=var_netop;-- asignar nuevo salaro mayor encontrado
END IF;
SET i=i+1;-- contador
END WHILE;
CLOSE c1;
...
(10, 9, '2023-08-08', 'credito', 281700, '2023-09-07', 231, 313000, 's'),
...
(366, 1, 1, '2023-08-06', '2023-08-07', 200000, 'n', 0),
(367, 1, 2, '2023-08-06', '2023-08-07', 200000, 'n', 0),
...
(378, 1, 13, '2023-08-09', '2023-08-09', 350000, 'n', 0),
...
(385, 1, 20, '2023-08-10', '2023-08-10', 370000, 'n', 0),
...
(392, 1, 21, '2023-08-13', '2023-08-13', 320000, 'n', 0),
...
(399, 1, 28, '2023-08-14', '2023-08-14', 270000, 'n', 0),
...
(406, 1, 35, '2023-08-15', '2023-08-15', 320000, 'n', 0),
...
(411, 1, 1, '2023-08-06', '2023-08-07', 200000, 'n', 0),
(412, 1, 2, '2023-08-06', '2023-08-07', 200000, 'n', 0),
...
(423, 1, 13, '2023-08-09', '2023-08-09', 350000, 'n', 0),
...
(430, 1, 20, '2023-08-10', '2023-08-10', 370000, 'n', 0),
...
(437, 1, 21, '2023-08-13', '2023-08-13', 320000, 'n', 0),
...
(444, 1, 28, '2023-08-14', '2023-08-14', 270000, 'n', 0),
...
(451, 1, 35, '2023-08-15', '2023-08-15', 320000, 'n', 0),
...
CREATE TABLE `empleado` (
  ...
  `telefono` varchar(25) NOT NULL,
  `fecha_ingreso` date DEFAULT curdate(),
  `estado_civil` enum('soltera','soltero','casado','casada','divorciado','divorciada','union libre','viudo') DEFAULT NULL,
...
(21, '2023-09-13', 2, 1, 'credito', 3),
(22, '2023-08-12', 2, 1, 'efectivo', 2);
...
(80, '2023-10-25', 9, 6, 'credito', 1);

--
-- Disparadores `factura_cabeza`
--
DELIMITER $$
CREATE TRIGGER `actualiza_catera` AFTER INSERT ON `factura_cabeza` FOR EACH ROW BEGIN
IF NEW.forma_pago="credito" AND NEW.tipomov_cod=1 THEN -- si es una transaccion de venta=1 y es credito
...
(104, 40, 1, 50, 3000, 0.10),
(105, 40, 2, 50, 8000, 0.10),
(106, 40, 3, 50, 3800, 0.10),
(107, 40, 4, 50, 3000, 0.10),
(108, 42, 5, 50, 7500, 0.20),
(109, 42, 6, 50, 3500, 0.20),
(110, 42, 7, 50, 5500, 0.20),
(111, 42, 8, 50, 7150, 0.20),
(112, 41, 1, 50, 3000, 0.10),
(113, 80, 2, 50, 8000, 0.10),
(114, 41, 3, 50, 3800, 0.10),
(115, 41, 4, 50, 3000, 0.10),
(116, 80, 1, 50, 3000, 0.10),
(117, 80, 2, 50, 8000, 0.10),
(118, 80, 3, 50, 3800, 0.10),
(119, 80, 4, 50, 3000, 0.10);
...
(1, 'Galletas Festival Bsax12', 2000, 3000, 4130, 4089, '2023-04-01', '2024-04-01', 3, 2),
(2, 'Galletas Ducales taco', 7000, 8000, 6075, 4088, '2023-04-01', '2024-04-01', 3, 2),
(3, 'Bom bom bum barrax50', 3000, 3800, 4312, 4081, '2023-05-01', '2024-05-01', 4, 3),
(4, 'Pan Blanco tajado', 4500, 5500, 6173, 4080, '2023-02-01', '2024-02-01', 2, 4),
(5, 'Salsa de tomate frasco', 7150, 7500, 7173, 4084, '2023-06-01', '2024-06-01', 5, 7),
(6, 'Jugo Fresa frasco', 2500, 3500, 6430, 4086, '2023-03-01', '2024-03-01', 6, 6),
(7, 'Leche pasteurizada bsa', 4500, 5500, 7524, 4080, '2023-03-01', '2024-03-01', 1, 5),
(8, 'ranchera', 5500, 7150, 10195, 4070, '2023-08-30', '2023-09-20', 7, 8),
...
,`cod_cargo` int(11)
,`tipo_documento` enum('cc','ce','nit','rut')
...
--
-- Estructura Stand-in para la vista `vista_categoria_productos`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_categoria_productos` (
...
--
-- Estructura Stand-in para la vista `vista_proveedor_productos`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_proveedor_productos` (
...
--
-- Estructura Stand-in para la vista `vista_tipomov_entrada_cabeza`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_tipomov_entrada_cabeza` (
`cod_tipomov` int(11)
...
--
-- Estructura Stand-in para la vista `vista_tipomov_factura_cabeza`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_tipomov_factura_cabeza` (
`cod_tipomov` int(11)
,`nombre_mov` varchar(25)
,`cod_factura` int(11)
,`empleado_cod` int(11)
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_cargo_empleado`  AS SELECT `t1`.`nombre_cargo` AS `nombre_cargo`, `t1`.`cod_cargo` AS `cod_cargo`, `t2`.`tipo_documento` AS `tipo_documento`, `t2`.`no_documento` AS `no_documento`, `t2`.`cod_empleado` AS `cod_empleado` FROM (`cargo` `t1` join `empleado` `t2`) ;

-- --------------------------------------------------------

--
-- Estructura para la vista `vista_categoria_productos`
--
DROP TABLE IF EXISTS `vista_categoria_productos`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_categoria_productos`  AS SELECT `t1`.`nombre_categoria` AS `nombre_categoria`, `t2`.`descripcion` AS `descripcion`, `t2`.`valor_compra` AS `valor_compra`, `t2`.`valor_venta` AS `valor_venta`, `t2`.`existencia` AS `existencia` FROM (`categoria` `t1` join `productos` `t2`) WHERE `t1`.`cod_categoria` = `t2`.`categor_cod` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_empleado_cliente`  AS SELECT `t1`.`cod_empleado` AS `cod_empleado`, `t1`.`nombre1` AS `nombre1`, `t2`.`apellido1` AS `apellido1`, `t2`.`sexo` AS `sexo` FROM (`empleado` `t1` join `cliente` `t2`) ;

-- --------------------------------------------------------

--
-- Estructura para la vista `vista_proveedor_productos`
--
DROP TABLE IF EXISTS `vista_proveedor_productos`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_proveedor_productos`  AS SELECT `t1`.`razon_social` AS `razon_social`, `t1`.`ciudad` AS `ciudad`, `t2`.`descripcion` AS `descripcion`, `t2`.`valor_compra` AS `valor_compra`, `t2`.`valor_venta` AS `valor_venta`, `t2`.`existencia` AS `existencia` FROM (`proveedor` `t1` join `productos` `t2`) WHERE `t1`.`cod_proveedor` = `t2`.`cod_producto` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `vista_tipomov_entrada_cabeza`
--
DROP TABLE IF EXISTS `vista_tipomov_entrada_cabeza`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_tipomov_entrada_cabeza`  AS SELECT `t1`.`cod_tipomov` AS `cod_tipomov`, `t1`.`nombre_mov` AS `nombre_mov`, `t2`.`cod_entrada` AS `cod_entrada`, `t2`.`fecha_entrada` AS `fecha_entrada` FROM (`tipomov` `t1` join `entrada_cabeza` `t2`) WHERE `t1`.`cod_tipomov` = `t2`.`tipomov_cod` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `vista_tipomov_factura_cabeza`
--
DROP TABLE IF EXISTS `vista_tipomov_factura_cabeza`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_tipomov_factura_cabeza`  AS SELECT `t1`.`cod_tipomov` AS `cod_tipomov`, `t1`.`nombre_mov` AS `nombre_mov`, `t2`.`cod_factura` AS `cod_factura`, `t2`.`empleado_cod` AS `empleado_cod` FROM (`tipomov` `t1` join `factura_cabeza` `t2`) WHERE `t1`.`cod_tipomov` = `t2`.`tipomov_cod` ;
...
ALTER TABLE `productos`
  ...
  ADD KEY `descripcon` (`descripcion`),
...
--
ALTER TABLE `entrada_detalle`
  MODIFY `cod_edetalle` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=54;

--
-- AUTO_INCREMENT de la tabla `factura_cabeza`
--
ALTER TABLE `factura_cabeza`
  MODIFY `cod_factura` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=81;

--
-- AUTO_INCREMENT de la tabla `factura_detalle`
--
ALTER TABLE `factura_detalle`
  MODIFY `cod_facdetalle` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=120;
...
-- Eventos
--
CREATE DEFINER=`root`@`localhost` EVENT `actuliza_estadocartera` ON SCHEDULE EVERY 1 SECOND STARTS '2024-05-30 08:17:45' ON COMPLETION PRESERVE ENABLE DO CALL actualizar_credito$$

CREATE DEFINER=`root`@`localhost` EVENT `event_netocartera` ON SCHEDULE EVERY 1 SECOND STARTS '2024-05-30 09:43:41' ON COMPLETION PRESERVE ENABLE DO CALL actualiza_netocartera$$
